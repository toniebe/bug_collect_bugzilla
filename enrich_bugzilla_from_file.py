...
FILTER_REQUIRE_COMMIT = os.getenv("FILTER_REQUIRE_COMMIT","0") not in ("0","false","False")
...
def enrich_one(bug):
    ...
    commits=set(); files=set()

    if FETCH_COMMENTS:
        try:
            for txt in fetch_comments(bid):
                commits.update(extract_commit_refs(txt))
    ...
    if FETCH_ATTACHMENTS:
        try:
            ...
            for att in metas:
                ...
                if not looks_like_code_attachment(att): continue
    ...
    out = clean_bug(bug, commits, files)
    if FILTER_REQUIRE_COMMIT and not out["commit_refs"]:
        return None
    return out
